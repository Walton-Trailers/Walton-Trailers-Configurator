...
export async function registerRoutes(app: Express): Promise<Express> {
  ...
  // ========== Trailer Lengths & Pull Types Routes ==========
  
  // Get all trailer lengths with model information
  app.get("/api/trailer-lengths", requireAuth, async (req, res) => {
    try {
      const lengths = await storage.getAllTrailerLengths();
      res.json(lengths);
    } catch (error) {
      console.error("Error fetching trailer lengths:", error);
      res.status(500).json({ error: "Failed to fetch trailer lengths" });
    }
  });

  // Get trailer lengths by model ID
  app.get("/api/models/:modelId/lengths", requireAuth, async (req, res) => {
    try {
      const modelId = parseInt(req.params.modelId);
      if (isNaN(modelId)) {
        return res.status(400).json({ error: "Invalid model ID" });
      }
      const lengths = await storage.getTrailerLengthsByModel(modelId);
      res.json(lengths);
    } catch (error) {
  ...
  // Create a new trailer length
  app.post("/api/trailer-lengths", requireAuth, async (req, res) => {
    try {
      const { modelId, length, pullType } = req.body;
      
      if (!modelId || !length || !pullType) {
        return res.status(400).json({ error: "Model ID, length, and pull type are required" });
      }

      const newLength = await storage.createTrailerLength({
        modelId: parseInt(modelId),
        length: parseInt(length),
        pullType: pullType.trim()
      });
      
      res.status(201).json(newLength);
    } catch (error) {
      console.error("Error creating trailer length:", error);
      res.status(500).json({ error: "Failed to create trailer length" });
    }
  });

  // Update a trailer length
  app.patch("/api/trailer-lengths/:id", requireAuth, async (req, res) => {
    try {
      const lengthId = parseInt(req.params.id);
      if (isNaN(lengthId)) {
        return res.status(400).json({ error: "Invalid length ID" });
      }

      const updates: { length?: number; pullType?: string; isArchived?: boolean } = {};
      
      if (req.body.length !== undefined) {
        updates.length = parseInt(req.body.length);
      }
      if (req.body.pullType !== undefined) {
        updates.pullType = req.body.pullType.trim();
      }
      if (req.body.isArchived !== undefined) {
        updates.isArchived = req.body.isArchived;
      }

      const updatedLength = await storage.updateTrailerLength(lengthId, updates);
      res.json(updatedLength);
    } catch (error) {
      console.error("Error updating trailer length:", error);
      res.status(500).json({ error: "Failed to update trailer length" });
    }
  });

  // Delete a trailer length
  app.delete("/api/trailer-lengths/:id", requireAuth, async (req, res) => {
    try {
      const lengthId = parseInt(req.params.id);
      if (isNaN(lengthId)) {
        return res.status(400).json({ error: "Invalid length ID" });
      ...
      await storage.deleteTrailerLength(lengthId);
      res.json({ message: "Trailer length deleted successfully" });
    } catch (error) {
      console.error("Error deleting trailer length:", error);
      res.status(500).json({ error: "Failed to delete trailer length" });
    }
  });

  // Archive a trailer length
  app.patch("/api/trailer-lengths/:id/archive", requireAuth, async (req, res) => {
    try {
      const lengthId = parseInt(req.params.id);
      if (isNaN(lengthId)) {
        return res.status(400).json({ error: "Invalid length ID" });
      ...
      const archivedLength = await storage.archiveTrailerLength(lengthId);
      res.json(archivedLength);
    } catch (error) {
      console.error("Error archiving trailer length:", error);
      res.status(500).json({ error: "Failed to archive trailer length" });
    }
  });

  // Restore a trailer length
  app.patch("/api/trailer-lengths/:id/restore", requireAuth, async (req, res) => {
    try {
      const lengthId = parseInt(req.params.id);
      if (isNaN(lengthId)) {
        return res.status(400).json({ error: "Invalid length ID" });
      ...
      const restoredLength = await storage.restoreTrailerLength(lengthId);
      res.json(restoredLength);
    } catch (error) {
      console.error("Error restoring trailer length:", error);
      res.status(500).json({ error: "Failed to restore trailer length" });
    }
  });
